  ...
  const divisions = {
    ...
    'ag-monitors-ganshorn': {
      ...
      fields: [
        {key:'dateOfDespatch', label:'Date of Despatch', type:'date'},
        {key:'status', label:'Status', type:'select', options:STATUS_OPTIONS},
        {key:'model', label:'Model', type:'text'},
        {key:'descriptionOfGoods', label:'Description of Goods', type:'textarea'},
        {key:'qty', label:'Qty', type:'number'},
        {key:'oa', label:'OA', type:'text'},
        {key:'customerAddress', label:'Customer Address', type:'textarea'},
        {key:'remarks', label:'Remarks', type:'textarea'},
      ]
    },
    'shipl': {
  ...
  const API_BASE = 'http://localhost:3001/api/accout';
  ...
  const accoutForm = document.getElementById('accOutForm');
  ...
  function buildForm() {
    ...
    modalTitle.textContent = `Add New ACC-OUT`;
  ...
  function closeModal(){ modalOverlay.classList.remove('visible'); accoutForm.reset(); }
  ...
  accoutForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    const btn = accoutForm.querySelector('button[type="submit"]');
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (h1) h1.textContent = 'ACC-OUT - ' + activeDivision;
