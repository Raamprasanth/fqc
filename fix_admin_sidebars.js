...
// The correct sidebar from admin.html
...
const pagesToFix = [
  {file: 'ashort.html', activePage: 'admin-shortshipment'},
  {file: 'admin-out.html', activePage: 'admin-out'}
];
  ...
  if(fs.existsSync(path)) {
    ...
    if(start !== -1 && end !== -1) {
      let activeSidebar = sidebarTemplate.replace(
        new RegExp(\`<a href="[^"]+" data-page="\${page.activePage}">\`),
        (match) => match.replace('data-page=', 'class="active" data-page=')
      );
      ...
      console.log(\`Could not find sidebar block in \${page.file}\`);
    }
  }
});
