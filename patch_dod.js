...
let html = fs.readFileSync('public/frontend/dod.html', 'utf8');
...
const newScript = `<script>
  ...
  const divisions = {
    'ag-monitors-ganshorn': {
      ...
      fields: [
        ...
        {key:'unitSlNo', label:'Unit Sl.No', type:'text'},
        {key:'conf', label:'Conf', type:'text'},
        {key:'version', label:'Version', type:'text'},
        {key:'invoiceNoDate', label:'Invoice No and Date', type:'text'},
        {key:'problemDetail', label:'Problem Detail', type:'textarea'},
        {key:'actionTakenByService', label:'Action Taken by Service', type:'textarea'},
      ]
    },
    'vent-anesthesia': {
      ...
      fields: [
        ...
        {key:'invoiceNoDate', label:'Invoice No / Date', type:'text'},
        {key:'supplier', label:'Supplier', type:'text'},
        {key:'problemObservationDetails', label:'Problem Observation Details', type:'textarea'},
        {key:'actionTakenByService', label:'Action Taken by Service', type:'textarea'},
        {key:'returnDateAfterService', label:'Return Date After Service/Supplier', type:'date'},
        {key:'fqcObservation', label:'FQC Observation', type:'textarea'},
    ...
    'shipl': {
      label: 'SHIPL',
      fields: [
        {key:'dodNo', label:'DOD No', type:'text'},
        {key:'dodRaisedDate', label:'DOD Raised Date', type:'date'},
        {key:'status', label:'Status', type:'select', options:STATUS_OPTIONS},
        {key:'model', label:'Model', type:'text'},
        {key:'unitSlNo', label:'Unit.Sl.No', type:'text'},
        ...
        {key:'problemType', label:'Problem Type', type:'text'},
        {key:'unitReturnedDate', label:'Unit Returned Date', type:'date'},
        {key:'problemAnalysedByProduction', label:'Problem Analysed by Production', type:'textarea'},
        {key:'actionTakenByProduction', label:'Action Taken by Production', type:'textarea'},
        {key:'remarks', label:'Remarks', type:'textarea'},
      ]
    }
  };
  ...
  const API_BASE = 'http://localhost:3001/api/dod';
  ...
  const dodForm = document.getElementById('dodForm');
  ...
  function buildForm() {
    ...
    modalTitle.textContent = \`Add New DOD\`;
  ...
  function closeModal(){ modalOverlay.classList.remove('visible'); dodForm.reset(); }
  ...
  dodForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    const btn = dodForm.querySelector('button[type="submit"]');
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (h1) h1.textContent = 'DOD - ' + activeDivision;
...
fs.writeFileSync('public/frontend/dod.html', html);
console.log('Patched dod.html');
