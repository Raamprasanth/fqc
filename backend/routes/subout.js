...
const Subout = require('../models/Subout');
...
router.get('/', async (req, res) => {
  try {
    ...
    const records = await Subout.find(filter).sort({ createdAt: -1 });
    res.json(records);
  } catch (err) {
    console.error('[Get Subout Error]', err);
...
router.post('/', async (req, res) => {
  try {
    ...
    if (Array.isArray(req.body)) {
      ...
      const inserted = await Subout.insertMany(dataList);
      return res.status(201).json(inserted);
    }

    const data = userId ? { ...req.body, createdBy: userId } : req.body;
    const newRecord = await Subout.create(data);
    res.status(201).json(newRecord);
  } catch (err) {
    console.error('[Create Subout Error]', err);
