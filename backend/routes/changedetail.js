...
router.get('/', async (req, res) => {
  try {
    ...
    const records = await Changedetail.find(filter).sort({ createdAt: -1 });
    res.json(records);
  } catch (err) {
    console.error('[Get Changedetail Error]', err);
...
router.post('/', async (req, res) => {
  try {
    ...
    if (Array.isArray(req.body)) {
      ...
      const inserted = await Changedetail.insertMany(dataList);
      return res.status(201).json(inserted);
    }

    const data = userId ? { ...req.body, createdBy: userId } : req.body;
    const newRecord = await Changedetail.create(data);
    res.status(201).json(newRecord);
  } catch (err) {
    console.error('[Create Changedetail Error]', err);
